// -----------------------------------------------------------------------
// <copyright file="OpenAIAIProviderTests.cs" company="Sassy Solutions">
...
{
    [Fact]
    public void ProviderId_Always_ReturnsOpenai()
    {
        // Arrange
    ...
    {
        // Arrange
        var (httpClient, handler) = TestFactories.CreateHttpClient(o => o.DefaultModel = "gpt-4o");
        var sut = TestFactories.CreateProvider(httpClient, o => o.DefaultModel = "gpt-4o");
        ...
        handler.When(HttpMethod.Post, "*/chat/completions")
            ...
            .Respond("application/json", """{"id":"x","model":"gpt-4o","created":0,"choices":[]}""");
        ...
        capturedBody!.Should().Contain("\"model\":\"gpt-4o\"");
    }
    ...
    {
        // Arrange
        var (httpClient, handler) = TestFactories.CreateHttpClient(o => o.DefaultModel = "gpt-4o-mini");
        var sut = TestFactories.CreateProvider(httpClient, o => o.DefaultModel = "gpt-4o-mini");
        string? body = null;
        handler.When(HttpMethod.Post, "*/chat/completions")
            .With(req => { body = req.Content!.ReadAsStringAsync().GetAwaiter().GetResult(); return true; })
    ...
    public async Task EmbedAsync_LargeInputs_BatchesByMaxEmbeddingsBatchSize()
    {
        ...
        {
            callCount++;
            // Return a single embedding per call regardless of the batch size — we only care about
            // call accounting and global index re-mapping. The handler sees individual requests.
    ...
    {
        // Arrange
        var (httpClient, handler) = TestFactories.CreateHttpClient(o => o.DefaultEmbeddingModel = "text-embedding-3-large");
        var sut = TestFactories.CreateProvider(httpClient, o => o.DefaultEmbeddingModel = "text-embedding-3-large");
        string? body = null;
        ...
        // Assert
        result.IsSuccess.Should().BeTrue();
        body!.Should().Contain("text-embedding-3-large");
    ...
    public async Task ListModelsAsync_DefaultsProviderWhenOwnerOmitted()
    {
        ...
        // Assert
        result.IsSuccess.Should().BeTrue();
        result.Value[0].Provider.Should().Be("openai");
    }
