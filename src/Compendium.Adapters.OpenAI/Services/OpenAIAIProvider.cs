// -----------------------------------------------------------------------
// <copyright file="OpenAIAIProvider.cs" company="Sassy Solutions">
...
namespace Compendium.Adapters.OpenAI.Services;

/// <summary>
/// OpenAI implementation of <see cref="IAIProvider"/>. Provides chat completions, streaming,
/// embeddings, tool calling, and structured outputs against the OpenAI REST API.
/// </summary>
internal sealed class OpenAIAIProvider : IAIProvider
{
        ...
        CancellationToken cancellationToken = default)
    {
        ...
        _logger.LogDebug("Sending OpenAI chat completion to model {Model}", model);
    ...
    {
        var model = string.IsNullOrEmpty(request.Model) ? _options.DefaultModel : request.Model;
        _logger.LogDebug("Sending OpenAI streaming chat completion to model {Model}", model);
        ...
        CancellationToken cancellationToken = default)
    {
        ...
        _logger.LogDebug(
            "Sending OpenAI embeddings request for {Count} inputs (batch size {Batch}, model {Model})",
        ...
        for (var offset = 0; offset < request.Inputs.Count; offset += batchSize)
        {
            ...
            var batchRequest = new OpenAIEmbeddingsRequest
            {
                Model = model,
                Input = slice,
    ...
    public async Task<Result<IReadOnlyList<AIModel>>> ListModelsAsync(
        CancellationToken cancellationToken = default)
    {
        _logger.LogDebug("Fetching available models from OpenAI");
    ...
    public async Task<Result> HealthCheckAsync(CancellationToken cancellationToken = default)
    {
        ...
        {
            _logger.LogWarning(ex, "Health check failed for OpenAI provider");
            return Result.Failure(AIErrors.ProviderUnavailable("openai"));
        }
    }

    private OpenAIChatCompletionRequest MapToApiRequest(CompletionRequest request, string model, bool stream)
    {
        ...
        var apiRequest = new OpenAIChatCompletionRequest
        {
            Model = model,
            Messages = messages,
    ...
    private static void ApplyTools(OpenAIChatCompletionRequest apiRequest, CompletionRequest request)
    {
        ...
        if (request.AdditionalParameters.TryGetValue(OpenAIToolCallingExtensions.ToolsKey, out var toolsRaw)
            && toolsRaw is IReadOnlyList<AgentTool> tools
            && tools.Count > 0)
        {
            apiRequest.Tools = tools.Select(t => new OpenAIToolDefinition
            {
        ...
        if (request.AdditionalParameters.TryGetValue(OpenAIToolCallingExtensions.ToolChoiceKey, out var choiceRaw)
    ...
    private void ApplyResponseFormat(OpenAIChatCompletionRequest apiRequest, CompletionRequest request)
    {
        var parameters = request.AdditionalParameters;
        if (parameters != null
            && parameters.TryGetValue(OpenAIStructuredOutputExtensions.SchemaKey, out var schemaRaw)
            && schemaRaw is string schemaJson
            && !string.IsNullOrWhiteSpace(schemaJson))
        {
            var schemaName = parameters.TryGetValue(OpenAIStructuredOutputExtensions.SchemaNameKey, out var nameRaw)
                && nameRaw is string s
                ? s
                : "response";
            var strict = !parameters.TryGetValue(OpenAIStructuredOutputExtensions.StrictKey, out var strictRaw)
                || strictRaw is not bool b
                || b;

            apiRequest.ResponseFormat = new OpenAIResponseFormat
            {
                Type = "json_schema",
        ...
        var explicitJsonMode = parameters != null
            && parameters.TryGetValue(OpenAIStructuredOutputExtensions.JsonModeKey, out var jsonModeRaw)
            && jsonModeRaw is bool jsonModeFlag
            && jsonModeFlag;

        if (explicitJsonMode || _options.UseStructuredOutputsByDefault)
        {
            apiRequest.ResponseFormat = new OpenAIResponseFormat { Type = "json_object" };
        }
    }
    ...
    private static CompletionResponse MapToCompletionResponse(OpenAIChatCompletionResponse apiResponse)
    {
        ...
        if (message?.ToolCalls != null && message.ToolCalls.Count > 0)
        {
            ...
            {
                [OpenAIToolCallingExtensions.ToolCallsMetadataKey] = invocations
            };
        }
    ...
    private static AgentToolInvocation MapToAgentToolInvocation(OpenAIToolCall toolCall)
    {
    ...
    private static CompletionChunk MapToCompletionChunk(OpenAIStreamChunk chunk, int index)
    {
    ...
    private static AIModel MapToAIModel(OpenAIModelInfo model)
    {
        ...
        return new AIModel
        {
            Id = model.Id,
            Name = model.Id,
            Provider = model.OwnedBy ?? "openai",
            SupportsStreaming = supportsChat,
            SupportsEmbeddings = supportsEmbeddings,
            SupportsVision = model.Id.Contains("gpt-4o", StringComparison.OrdinalIgnoreCase)
                || model.Id.Contains("vision", StringComparison.OrdinalIgnoreCase),
